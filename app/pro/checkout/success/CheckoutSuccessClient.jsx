...
export default function CheckoutSuccessClient() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        const res = await fetch(`/api/pro-success?session_id=${encodeURIComponent(sid)}`);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ✅ 通知表示 */}
      {notice && (
        <div className="mt-4 rounded-lg border px-4 py-3 bg-green-50 text-green-700">
          {notice}
        </div>
      )}
        ...
        <button
          ...
          onClick={async () => {
            if (!result || typeof result !== "object") {
              setNotice("チェックデータが取得できていません。再読み込みしてください。");
              return;
            }
            ...
            try {
              ...
              setNotice("PDFをメール送信しました！");
            } catch (e) {
              console.error("❌ PDF送信エラー:", e);
              setNotice("送信に失敗しました: " + e.message);
            }
          }}
        >
          PDFをメール送信する
